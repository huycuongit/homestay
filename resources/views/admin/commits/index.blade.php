...
@section('content')
    ...
    <!-- URL action -->
    <input type="hidden" value="{{ route('admin.commits.datatables') }}" id="url-datatable" />
    <input type="hidden" value="{{ route('admin.commits.show', ['id' => '_id']) }}" id="url-show" />
    <input type="hidden" value="{{ route('admin.commits.edit', ['id' => '_id']) }}" id="url-edit" />
    <input type="hidden" value="{{ route('admin.commits.destroy', ['id' => '_id']) }}" id="url-destroy" />
    <input type="hidden" value="{{ route('admin.commits.create') }}" id="url-create" />
        ...
        function getData() {
            ...
            var _ajax = {
                ...
                data: function(d) {
                    ...
                    d.quarter = $('#filter_quarter').val() ? $('#filter_quarter').val() : '';
            ...
            var _columns = [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var _actionMoreEdit = '';
                        if (permissions.includes('admin.commits.destroy')) {
                            _actionMoreDelete = `<a href="#" class="dropdown-item btn-destroy">Xoá</a>`;
                        }
                        if (permissions.includes('admin.commits.edit')) {
            ...
            if (permissions.includes('admin.commits.create')) {
                _buttons.push({
                    text: '<i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Tạo mới</span>',
                    className: 'add-new btn btn-primary waves-effect waves-light'
                });
            ...
            var _table = $(_idDatatables).DataTable({
                ...
                columnDefs: [
                    {
                        targets: 3, // Cột description (đếm từ 0)
                        className: 'col-description-limit'
                    }
                ]
            });
            ...
            $(_idDatatables).on('click', '.btn-destroy', function() {
                var data = _table.row($(this).parents('tr')).data();
                var _itemId = data.id;
                $('.modal-title-destroy').html(`Bạn có chắc chắn muốn xóa cam kết? <br> "${data.namem}"`)
