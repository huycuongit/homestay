...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <form class="needs-validation" novalidate enctype="multipart/form-data" method="POST" id="form-form"
            action="{{ isset($data) ? route('admin.commits.update', ['id' => $data->id]) : route('admin.commits.store') }}">
            <div class="row">
                <div class="col-md-8">
                    <h3>{{ isset($data) ? (request()->has('is_view') ? 'Thông tin cam kết' : 'Cập nhật cam kết') : 'Tạo mới cam kết' }}
                    </h3>
                </div>
                @if (!request()->has('is_view'))
                    <div class="col-md-4">
                        <button type="submit" class="btn btn-primary waves-effect waves-light float-end">Lưu</button>
                        <button type="button" data-url="{{ route('admin.commits.index') }}"
                ...
                @else
                    @if ($data->booking_future <= 0)
                        <div class="col-md-4">
                            <button type="button" data-url="{{ route('admin.commits.edit', ['id' => $data->id]) }}"
            ...
            <div class="row mb-6" style="pointer-events: {{ request()->has('is_view') ? 'none' : 'unset' }}">
                ...
                <div class="col-12" style="pointer-events: {{ request()->has('is_view') ? 'none' : 'unset' }}">
                    <div class="card mb-6">
                        ...
                        <h6 class="card-header">Icon<span class="text-danger">*</span></h6>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <!-- Dropzone container -->
                                    <div action="/upload" class="dropzone needsclick" id="dropzone-avatar"
                                        data-hidden-input-id="icon"
                                    ...
                                    <input type="hidden" id="icon" name="icon"
                                        value="{{ old('icon', isset($data) && $data->icon ? Storage::url($data->icon) : '') }}" />
                                </div>
                                <div class="col-md-8">
                                    <div class="col-md-12">
                                        <div class="row">
                                            <div class="col-md-6 mb-6">
                                                ...
                                                <input type="text" class="form-control" id="name" name="name"
                                                    ...
                                                    placeholder="Tên cam kết" aria-label="Tên cam kết" />
                                            </div>
                    ...
                    fv = FormValidation.formValidation(_form, {
                        fields: {
                            name: {
                                validators: {
                                    notEmpty: {
                                        message: 'Vui lòng nhập tên cam kết'
                                    }
                                }
                            },
                            icon: {
