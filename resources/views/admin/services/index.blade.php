...
@section('content')
    ...
    <!-- URL action -->
    <input type="hidden" value="{{ route('admin.services.datatables') }}" id="url-datatable" />
    <input type="hidden" value="{{ route('admin.services.show', ['id' => '_id']) }}" id="url-show" />
    <input type="hidden" value="{{ route('admin.services.edit', ['id' => '_id']) }}" id="url-edit" />
    <input type="hidden" value="{{ route('admin.services.destroy', ['id' => '_id']) }}" id="url-destroy" />
    <input type="hidden" value="{{ route('admin.services.create') }}" id="url-create" />
...
@push('js')
    ...
    <script>
        function getData() {
            ...
            var _columns = [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var _actionMoreEdit = '';
                        if (permissions.includes('admin.services.destroy')) {
                            _actionMoreDelete = `<a href="#" class="dropdown-item btn-destroy">Xoá</a>`;
                        }
                        if (permissions.includes('admin.services.edit')) {
            ...
            if (permissions.includes('admin.services.create')) {
                _buttons.push({
                    text: '<i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Tạo mới</span>',
                    className: 'add-new btn btn-primary waves-effect waves-light'
                });
