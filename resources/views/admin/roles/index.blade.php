...
@section('content')
    ...
    <input type="hidden" value="{{ route('admin.roles.create') }}" id="url-create" />
    <input type="hidden" value="{{ route('admin.roles.edit', ['id' => '_id']) }}" id="url-edit" />
    <input type="hidden" value="{{ route('admin.roles.update', ['id' => '_id']) }}" id="url-update" />
    <input type="hidden" value="{{ route('admin.roles.store') }}" id="url-store" />
...
@push('js')
    ...
    <script>
        function getData() {
            ...
            var _columns = [{
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        return `<a href="${_actionEditIdURL}?is_view=true" class="text-primary btn-edit">${data}</a>`;
                    }
                },
                {
                    "data": "created_name"
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var _actionEdit =
                            permissions.includes('admin.roles.edit') ?
                        ...
                        var _actionDelete =
                            permissions.includes('admin.roles.destroy') ?
            ...
            if (permissions.includes('admin.roles.create')) {
                _buttons.push({
            ...
            $('.move-edit').on('click', function(e) {
                ...
                $('#offcanvasFormLabel').text('Chỉnh sửa thông tin phòng ban');
            ...
            $(_idDatatables).on('click', '.btn-destroy', function() {
                var data = _table.row($(this).parents('tr')).data();
                var _itemId = data.id;
                $('.modal-title-destroy').html(`Bạn có chắc chắn muốn xóa phòng ban? <br> "${data.name}"`)
