...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <form class="needs-validation" novalidate enctype="multipart/form-data" method="POST" id="form-form"
            action="{{ isset($data) ? route('admin.images.update', ['id' => $data->id]) : route('admin.images.store') }}">
            <div class="row">
                <div class="col-md-8">
                    <h3>{{ isset($data) ? (request()->has('is_view') ? 'Thông tin hình ảnh' : 'Cập nhật hình ảnh') : 'Tạo mới hình ảnh' }}
                    </h3>
                </div>
                @if (!request()->has('is_view'))
                    <div class="col-md-4">
                        <button type="submit" class="btn btn-primary waves-effect waves-light float-end">Lưu</button>
                        <button type="button" data-url="{{ route('admin.images.index') }}"
                ...
                @else
                    @if ($data->booking_future <= 0)
                        <div class="col-md-4">
                            <button type="button" data-url="{{ route('admin.images.edit', ['id' => $data->id]) }}"
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <input type="hidden" id="url" name="url"
                                        value="{{ old('url', isset($data) && $data->url ? Storage::url($data->url) : '') }}" />
                                </div>
                                <div class="col-md-8">
                                    <div class="col-md-12">
                                        <div class="row">
                                            <div class="col-md-6 mb-4">
                                                <label class="form-label" for="gallery_id">
                                                    Danh mục <span class="text-danger">*</span>
                                                </label>
                                                <select class="form-select" id="filter_active" name="gallery_id" aria-label="Default select example">
                                                    <option value="" selected="">Chọn danh mục</option>
                                                    @if (isset($galleries) && count($galleries) > 0)
                                                        @foreach ($galleries as $keyL => $valueL)
                                                            <option value="{{ $valueL->id }}"
                                                                {{ !empty($data->gallery_id) && $valueL->id == $data->gallery_id ? 'selected' : (old('gallery_id') == $valueL->id ? 'selected' : '') }}>
                                                                {{ $valueL->name }}
                                                            </option>
                                                        @endforeach
                                                    @endif
                                            ...
                                            <div class="col-md-6 mb-6">
                                                ...
                                                <input type="text" class="form-control" id="name" name="name"
                                                    value="{{ isset($data) && $data->name ? $data->name : old('name') }}"
                                                    placeholder="Tên hình ảnh" aria-label="Tên hình ảnh" />
                                            </div>
                            ...
                            {{-- <div class="row mt-6">
                                <div class="col-md-12">
                                    <div class="col-12 mb-6">
                                        ...
                                        <textarea name="content" id="content" cols="30" rows="10" style="display: none">{!! $data['content'] ?? '' !!}</textarea>
                                        <div id="ckeditor_content">{!! $data['content'] ?? '' !!}
                                        </div>
                    ...
                    fv = FormValidation.formValidation(_form, {
                        fields: {
                            ...
                            publish_time: {
                                validators: {
                                    notEmpty: {
                                        message: 'Vui lòng nhập thời gian phát hành'
                                    },
                                    callback: {
                                        message: 'Thời gian phát hành phải lớn hơn thời gian hiện tại',
                                        callback: function(value, validator, $field) {
                                            ...
                                            if (!isEditMode) {
                                                ...
                                                const publishTime = new Date(value.value);
