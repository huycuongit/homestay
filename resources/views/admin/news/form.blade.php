...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <form class="needs-validation" novalidate enctype="multipart/form-data" method="POST" id="form-form"
            action="{{ isset($data) ? route('admin.news.update', ['id' => $data->id]) : route('admin.news.store') }}">
            <div class="row">
                <div class="col-md-8">
                    <h3>{{ isset($data) ? (request()->has('is_view') ? 'Thông tin bài viết' : 'Cập nhật bài viết') : 'Tạo mới bài viết' }}
                    </h3>
                </div>
                @if (!request()->has('is_view'))
                    <div class="col-md-4">
                        <button type="submit" class="btn btn-primary waves-effect waves-light float-end">Lưu</button>
                        <button type="button" data-url="{{ route('admin.news.index') }}"
                ...
                @else
                    @if ($data->booking_future <= 0)
                        <div class="col-md-4">
                            <button type="button" data-url="{{ route('admin.news.edit', ['id' => $data->id]) }}"
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <div action="/upload" class="dropzone needsclick" id="dropzone-avatar"
                                        ...
                                        style="pointer-events: {{ isset($data) && $data->booking_exits ? 'none' : 'unset' }}">
                                    ...
                                    <input type="hidden" id="avatar" name="avatar"
                                        value="{{ isset($data) && $data->avatar ? Storage::url($data->avatar) : '' }}" />
                                </div>
                                <div class="col-md-8">
                                    <div class="col-md-12">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 mb-6">
                                                <label for="publish_time" class="form-label">
                                                    Thời gian phát hành
                                                    <span class="text-danger">*</span>
                                                </label>
                                                <input type="datetime-local" class="form-control" id="publish_time"
                                                    name="publish_time"
                                                    value="{{ isset($data) && $data->publish_time ? $data->publish_time : old('publish_time') }}"
                                                    placeholder="Thời gian phát hành" aria-label="Thời gian phát hành" />
