...
class ProvinceRepository implements ProvinceRepositoryInterface
{
    ...
    public function getAllData($active = 1)
    {
        $sql = $this->model->select('*')->where('active', $active)->orderBy('order', 'asc')->get();
        return $sql ?? false;
    }
    ...
    {
        return $this->model->where('id', '<>', $id)->whereIn('phone_code', $phone_code)->first();
    }
}
